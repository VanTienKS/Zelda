                ...
                for col_index, col in enumerate(row):
                    if col != '-1':
                        ...
                        if style == 'grass':
                            ...
                            Tile((x, y), [
                                 self.visible_sprites, self.obstacles_sprites, self.attackable_sprites], 'grass', random_grass_img)
                        if style == 'object':
                        ...
                        if style == 'entities':
                            if col == '394':
                                self.player = Player(
                                    (x, y), [self.visible_sprites], self.obstacles_sprites, self.create_attack, self.destroy_weapon, self.create_magic)
                            else:
                                if col == '390':
                                    monster_name = 'bamboo'
                                elif col == '391':
                                ...
                                Enemy(monster_name, (x, y), [
                                      self.visible_sprites, self.attackable_sprites], self.obstacles_sprites, self.damage_player, self.trigger_death_particles, self.add_exp)
...
class YSortCameraGroup(pygame.sprite.Group):
    ...
    def custom_draw(self, player):
        ...
        for sprite in sorted(self.sprites(), key=lambda sprite: sprite.rect.centery):
            ...
            if (offset_rect[0] + sprite.image.get_width() > 0 and offset_rect[0] < SCREEN_WIDTH):
                if (offset_rect[1] + sprite.image.get_height() > 0 and offset_rect[1] < SCREEN_HEIGHT):
